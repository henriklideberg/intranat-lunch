...
// pages/api/cron.js (CR-safe version)
import * as cheerio from 'cheerio';

const VERSION = '2025-08-19-CRSAFE-1';
...
function linesFromHtml(html){
  ...
  // CR/LF-safe split – inga regex och inga "
" i källkoden
  const LF = String.fromCharCode(10); // 

  // Ersätt CR (13) med LF (10) och splitta på LF
  let normalized = '';
...
export default async function handler(req, res){
  try{
    ...
    if (!['måndag','tisdag','onsdag','torsdag','fredag'].includes(weekday)){
      return res.status(200).json({ skipped:true, reason:'helg', version: VERSION });
    }
